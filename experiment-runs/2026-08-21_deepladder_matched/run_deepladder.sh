#!/usr/bin/env bash
# Sharded driver for the deep-ladder matched-pool eval (EXPERIMENT_LOG 2026-08-21 #7).
...
# Every cell writes results/<cell_id>_deepladder.json. Nothing of record is
...
while IFS=$'\t' read -r tag ck cfg; do
  ...
  i=$((i+1))
  "$PY" deepladder_eval.py --ckpt "$ck" --tag "$tag" --cellcfg "$cfg" --outdir "$OUTDIR"
  rc=$?
