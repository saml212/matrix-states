...
def main():
    ...
    # ============================ §6.1 Curve 5a ===========================
    hdr("§6.1 CURVE 5a -- DEPTH DRIFT at 1.31B (median of per-seed kappa@11sq - kappa@5sq); "
        "band: within +/-0.05 of the 1.31B K=24 value")
    ...
    for readout, getter, ref_T, ref_U in (
            ...
            ("h_top (5 squarings, breadth battery)",
             None, None, None)):
        hdr(f"TEST-W -- within-1.31B freeze ordering, 4 strata, at {readout}")
        U392, U98, strata = [], [], []
        for k in KS:
            if getter:
