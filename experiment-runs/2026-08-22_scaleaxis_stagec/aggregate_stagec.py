#!/usr/bin/env python3
"""STAGE C AGGREGATOR -- every pinned test in NCR_SCALE_AXIS_DESIGN.md R2 §5/§6.
...
def main():
    ...
    D392 = load_depth(os.path.join(d, "depthext6_392m_*_depthext.json"), "392M")
    D98 = load_depth(os.path.join(d, "ref98m_depth", "depthext6_*_depthext.json"), "98M")
    B392 = load_battery([os.path.join(d, "*scaleaxis392m_*_kscaling.json")], "392M")
    B98 = load_battery([os.path.join(d, "ref98m_battery", "*_kscaling.json")], "98M")

    print(f"loaded: 392M depth {len(D392)}, 98M depth {len(D98)}, "
          f"392M battery {len(B392)}, 98M battery {len(B98)}  (expect 24 each)")
    bad = [k for k, v in D392.items() if v["rec"]["self_check"] != "PASS"
           or v["rec"]["ckpt_step"] != 20000]
    print("392M depth invalid:", bad if bad else "none")
    print("392M ladders:", {k: v["rec"]["depth_ladder"] for k, v in
                            sorted(D392.items()) if k[1] and k[2] == 0})
    for label, dd in (("392M", D392), ("98M", D98)):
    ...
    # ============================ §6.1 Curve 1 ============================
    hdr("§6.1 CURVE 1 -- CAPABILITY at h_top (5 squarings), P1b, 392M. Bar: kappa >= 0.90 on >=2/3 seeds")
    ...
    for readout, getter, ref_T, ref_U in (
        ...
        print(f"{'392M':>4s} " + " ".join(f"{u:8.1f}" for u in U392) + f" {T392:9.1f}")
        print(f"{'98M':>4s} " + " ".join(f"{u:8.1f}" for u in U98) + f" {T98:9.1f}")
        if ref_T is not None:
            ok = abs(T98 - ref_T) < 1e-9 and tuple(U98) == ref_U
            print(f"     [98M reference cross-check vs design §2.1 ({ref_U}, T={ref_T}): "
        ...
        print(f"descriptive cross-scale delta: T_W(392M) - T_W(98M) = {T392-T98:+.1f}")
        ...
        if T392 > 31.5 and nfail <= 1:
            v = "ORDERING-CONFIRMED (within-392M) / ORDERING-SCALE-STABLE"
            if T392 == 36:
    ...
    for name, get392, get98 in (
        ...
        hdr(f"TEST-X -- cross-scale, 8 strata (4K x 2 recipes). {name}")
        rows, strata, T = [], [], 0.0
        ties_tot = 0
        for k in KS:
            for fz in (True, False):
        ...
        print(f"{'K':>3s} {'recipe':>10s} {'392M kappa':>26s} {'98M kappa':>26s} {'U/9':>6s} {'ties':>5s}")
    ...
    for cname, delta, get392, get98 in (
        ...
        hdr(f"§6.2 CROSS-SCALE per K per recipe -- {cname}, delta = {delta}")
        print(f"{'K':>3s} {'recipe':>10s} {'392M med':>10s} {'98M med':>10s} {'D_scale':>10s} "
    ...
    hdr("Rule R-delta MECHANICS APPLIED TO 392M DATA -- DESCRIPTIVE ONLY, NOT A RE-ELECTION")
    ...
    print("\n(For the record, the SAME table on the 98M six-rung re-score is what elected")
    ...
    hdr("CROSS-SCALE DRIFT COMPARISON (median of per-seed kappa@11sq - kappa@5sq; "
