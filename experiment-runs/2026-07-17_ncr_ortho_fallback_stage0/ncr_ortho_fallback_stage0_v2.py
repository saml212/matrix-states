...
RUNNER_TAG = "ncr_ortho_fallback_stage0_v1"
        ...
        if self._orthogonal:
            if self._damped:
                ...
                with torch.no_grad():
                    ...
                    # finfo.tiny is a pure 0/0 guard that never interferes with
                    # the operating region: for any computed sigma_i >= tiny the
                    # division-then-multiplication cancels exactly at every
                    # magnitude, restoring the exact floor §2 requires.
                    scale = S_floor / S.clamp_min(torch.finfo(S.dtype).tiny)
