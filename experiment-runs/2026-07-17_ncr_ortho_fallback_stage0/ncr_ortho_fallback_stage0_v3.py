...
STAGE-0 PATCH (NCR_ORTHO_FALLBACK_DESIGN.md §2, commit 28e69cb FROZEN
CLEAR-FOR-STAGE-0-BUILD): adds a THIRD arm, 'damped_polar' -- identical to
'ortho' except a detached, no_grad SVD-floor correction is inserted before
the (unchanged) `newton_schulz_polar` call: `Z_damped = M @ Z_raw`, where
`M = U diag(max(sigma_i, eps_rel*sigma_max)/sigma_i) U^T` is built from
Z_raw's own SVD (no_grad) so `sigma_min(Z_damped) >= eps_rel*sigma_max(Z_raw)`
holds exactly, with a straight-through gradient to Z_raw (M is constant
w.r.t. autograd). `newton_schulz_polar` and everything downstream of
Z_damped are BYTE-IDENTICAL to the 'ortho' arm -- this is the ONLY semantic
change in this file relative to the pinned `ncr_ortho_write.py` (md5
83b5d7bd273e9e83698fed27a9f2ef45); the rest of the diff is cell-registry
(build_primary_model / primary_cell_id arm dispatch) and output-path
plumbing (RUNNER_TAG, --outdir default, --eps-rel CLI flag).
"""
...
RUNNER_TAG = "ncr_ortho_fallback_stage0_v3"
...
# Stage-0 SVD-floor patch default (NCR_ORTHO_FALLBACK_DESIGN.md §2, frozen
# 28e69cb): eps_rel is a FIXED dimensionless fraction of sigma_max, not
# learned, not annealed. 1e-3 is the pre-registered primary value; the
# branch logic's pre-authorized retries use 1e-2 (up) or 1e-4 (down), passed
# explicitly via --eps-rel, never by changing this default.
STAGE0_EPS_REL_DEFAULT = 1e-3
...
class NCROrthoWriteModel(els.NCREarlyLNModel):
    ...
    def __init__(self, d: int = nm.D_PIN, h: int = nm.ENC_H,
                 ...
                 damped: bool = False, eps_rel: float = STAGE0_EPS_REL_DEFAULT):
        super().__init__(d=d, h=h)
        self._orthogonal = bool(orthogonal)
        self._ns_iter = int(ns_iter)
        self._ns_power = int(ns_power)
        self._damped = bool(damped)              # NEW -- Stage-0 SVD-floor patch (§2)
        self._eps_rel = float(eps_rel)            # NEW -- Stage-0 SVD-floor patch (§2)

    def encode(self, keys, values):
        Z = self.encoder(keys, values)
        if self._orthogonal:
            if self._damped:
                # NCR_ORTHO_FALLBACK_DESIGN.md §2 -- detached SVD exact sigma_min
                # floor, straight-through gradient (M constant w.r.t. autograd:
                # dL/dZ_raw = M @ dL/dZ_damped). newton_schulz_polar below is
                # UNCHANGED -- it re-derives its own pre-scale sigma_hat from the
                # damped Z exactly as it would from any other Z.
                with torch.no_grad():
                    U, S, Vh = torch.linalg.svd(Z, full_matrices=False)
                    sigma_max = S[..., :1]
                    S_floor = S.clamp_min(self._eps_rel * sigma_max)
                    # [D1 FIX, §B4 audit 2026-07-17] denominator guard decoupled
                    # from NS_EPS (1e-7): NS_EPS sits exactly at §10.2's diagnosed
                    # danger threshold, so clamping the denominator there silently
                    # degraded the floor to proportional-in-sigma_min below 1e-7
                    # (1e-9 -> 100x short; ~0 -> 9 orders short, §B4 D1 table).
                    # [F-B FIX, §B6 Ruling 3, 2026-07-17] COMPOSED guard: the
                    # relative term eps_rel*sigma_max*1e-6 caps scale at 1e6,
                    # killing §B5 F-B (fp32 bit-exact-zero sigma -> scale ~1e36
                    # -> non-finite forward); the outer finfo.tiny backstop
                    # covers the measure-zero Z==0 case (where the relative
                    # term is itself 0, which would reintroduce 0/0). Floor is
                    # exact for every resolvable sigma_i above the bite point
                    # eps_rel*sigma_max*1e-6; a bounded, finite under-floor
                    # below it (see the design doc's §B6-scoped guarantee).
...
def build_primary_model(arm: str, d: int, h: int, ns_iter: int, ns_power: int,
                        eps_rel: float = STAGE0_EPS_REL_DEFAULT):
    """arm in {'free','ortho','damped_polar'}. 'free' is els.NCREarlyLNModel
    VERBATIM (the pinned §11.6 baseline); 'ortho' is the flag-on subclass;
    'damped_polar' is 'ortho' + the Stage-0 SVD-floor patch (§2). All three
    carry arm='ncr', _ln_alpha, encode/forward -- identical downstream
    pipeline."""
    if arm == "free":
    ...
    if arm == "damped_polar":            # NEW -- Stage-0 SVD-floor patch (§2)
        return NCROrthoWriteModel(d=d, h=h, orthogonal=True, damped=True,
                                  eps_rel=eps_rel, ns_iter=ns_iter, ns_power=ns_power)
...
def primary_cell_id(arm: str, K: int, seed: int) -> str:
    if arm == "damped_polar":            # NEW -- Stage-0 cell naming (§2)
        return f"stage0_damped_K{K}_s{seed}"
...
def run_primary_cell(arm: str, K: int, seed: int, steps: int, device: str,
                     ...
                     eps_rel: float = STAGE0_EPS_REL_DEFAULT) -> dict:
    ...
    assert arm in ("free", "ortho", "damped_polar"), arm
    ...
    model = build_primary_model(arm, d_eff, h_eff, ns_iter, ns_power, eps_rel=eps_rel).to(device)
    rec = dict(cell_id=cid, arm=arm, K=K, d=d_eff, h=h_eff, seed=seed,
               orthogonal=(arm in ("ortho", "damped_polar")),
               eps_rel=(eps_rel if arm == "damped_polar" else None),
               ns_iter=ns_iter, ns_power=ns_power,
...
def main():
    ...
    ap.add_argument("--arm", choices=("free", "ortho", "damped_polar"), default="ortho")
    ...
    ap.add_argument("--eps-rel", type=float, default=STAGE0_EPS_REL_DEFAULT,
                    help="Stage-0 SVD-floor relative epsilon (damped_polar arm only, §2)")
    ap.add_argument("--R", type=int, default=DISC_R)
    ap.add_argument("--outdir", type=str, default=os.path.join(_HERE, "results_ortho_fallback"))
    ...
    if args.primary_cell:
        rec = run_primary_cell(args.arm, args.K, args.seed, args.steps, args.device,
                               ...
                               anneal_frac=args.anneal_frac, eps_rel=args.eps_rel)
