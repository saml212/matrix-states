...
def _premise_valid_entry(entry, arm, th):
    ...
    recomputed from the RAW logged diagnostics against the run's own
    recorded thresholds -- never from the convenience booleans, so the rule
    applied here is exactly the pre-registered one even if a run's JSON
    predates a flag. Returns True/False, or None for the k<K collapse leg
    (the bound is never invoked there -- R2-1(iii); its numbers are
    diagnostics, not premise-gated headlines)."""
    ...
    if arm == "unconstrained":
        key_ok = entry.get("key_gram_deviation_mean", float("inf")) < th.get("tau_unconstrained", 0.03)
        val_ok = entry.get("value_gram_deviation_mean", float("inf")) < th.get("tau_value_unconstrained", 0.03)
    else:   # causal_k_ge_K: salvage tier or better (R2-1 ii), both sides
