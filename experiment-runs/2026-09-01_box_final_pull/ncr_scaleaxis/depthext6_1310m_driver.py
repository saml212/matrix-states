#!/usr/bin/env python3
"""SIX-RUNG DEPTH EXTENSION DRIVER, 1.31B SIDE -- squarings {5,7,9,11,13,15}.

NCR_SCALE_AXIS_DESIGN.md DRAFT-R2 sec 4.6 (Stage C) + sec 4.6.1's ladder table.

This is the 1.31B twin of `depthext6_driver.py` (which sec 3.5 confines to the
...
Usage (one cell):
  NCR_SCALE=1310m NCR_K=16 python3 depthext6_1310m_driver.py --k 16 --ckpt ... \\
      --tag depthext6_1310m_... [--cellcfg ...] [--outdir ...]
"""
...
# RE-PIN RULING OF RECORD: matrix-thinking/scaleaxis_build/EXPERIMENT_LOG.md
# "2026-08-25 #2" -- "the six-rung driver's PARENT_MD5 pins the kscaling
# unpatched copy; APPROVED to re-pin to the scaleaxis patched md5 e48eca71 FOR
# THE 1B RUN ONLY". NOTE (disclosed): this driver is a NEW file authored for the
# 1.31B run, a sibling of depthext6_392m_driver.py which ALREADY pinned
# e48eca71. So no audited file is edited and the kscaling-tree
# depthext6_driver.py (md5 8f222cd3, pinning e95ffe19) is left UNTOUCHED --
# the ruling's permission is recorded and cited, but the re-pin it authorises
# turned out not to be needed. Cited in every JSON record via `repin_ruling`.
REPIN_RULING = ("matrix-thinking/scaleaxis_build/EXPERIMENT_LOG.md 2026-08-25 #2 "
                "(re-pin to e48eca71 approved for the 1B run only); not exercised -- "
                "this 1310m driver is a new sibling file that already pins e48eca71, "
                "so the kscaling-tree depthext6_driver.py is unmodified")
...
def main() -> int:
    if os.environ.get("NCR_SCALE", "").strip().lower() != "1310m":
        print(f"!!! SCALE ABORT: NCR_SCALE={os.environ.get('NCR_SCALE')!r}, expected '1310m'. "
              f"This driver is the 1.31B side; the 98M side runs depthext6_driver.py from the "
    ...
    rec["driver"] = {
        "driver": "depthext6_1310m_driver.py",
        ...
        "repin_ruling": REPIN_RULING,
        "cross_scale_instrument_ruling": ("EXPERIMENT_LOG 2026-08-24 #2 Ruling 2 -- 1.31B verdicts "
                                          "use the SAME pairwise instruments as #21 on the "
                                          "(392M, 1.31B) pair, identical thresholds"),
