...
Writes sentinel .claude/.state/clean-ok-<hash> on zero-blocker pass.
...
def audit_markdown(f, is_new, repo_root):
    ...
    # Exempt: harness infrastructure (.claude/**) where .md files are
    # skill/agent definitions required to exist.
    if is_new and not f.startswith(".claude/"):
        issues.append((
            f, "blocker",
...
def compute_hash():
    r = run(["bash", ".claude/scripts/compute_clean_hash.sh"])
