...
# current prompt and inject the top N as stderr context so Claude sees them.
#
...
# Query FTS5, then fall back to recency if empty
...
if [ -z "$RESULTS" ]; then
  RESULTS=$("$SQLITE" "$DB" "SELECT '[' || category || '] ' || rule FROM learnings ORDER BY created_at DESC LIMIT 3;" 2>/dev/null)
fi
