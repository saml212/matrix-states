...
was negative: the constrained write did not train (FAIL, confirmed as
mechanism rather than implementation defect by the code re-audit
disclosed there). We report the odds we assigned before running it, not
after.
...
\textbf{Falsification, resolved.} The bound half of this paper's claim --
...
levers we did try did so. The mechanism half's pre-registered voiding
condition -- the fix landing NULL with a clean mechanistic signature
(departure-from-normality and condition number already near their healthy
targets) -- did not occur: the fix experiment
(Section~\ref{sec:pending}) landed FAIL, not NULL, and its signature is
degenerate and untrained (departure-from-normality $0.17$--$0.69$,
condition number $154$--$4387$), so the wall-mechanism diagnosis stands
unvoided -- though a FAIL, unlike a WIN, does not additionally confirm
it. The post-FAIL code re-audit has since closed MECHANISM-CONFIRMED, no
bug: the projection \emph{route} failed for a now-diagnosed, independent
trainability reason -- a tight-spare ($d{=}K{+}1$) ill-conditioning trap
in which the scale-invariant loss exerts no pressure on the spare
direction, $\sigma_{\min}$ collapses below the Newton--Schulz iteration's
working range, and the polar backward explodes as $\sim 1/\sigma_{\min}$
-- which neither confirms nor refutes the wall mechanism itself; the
orthogonal-operator hypothesis remains open for structurally different
parametrizations, and a Cayley/skew-symmetric fallback design is in
progress (\texttt{NCR\_ORTHO\_WRITE.md} \S10).
% evidence: NCR_ORTHO_WRITE.md S10.0/S10.7/S10.8
