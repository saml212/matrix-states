...
pre-register the fix experiment's verdict bands and report its status as
pending (Section~\ref{sec:pending}).
...
collapsing modulo the cycle length. (5) A pre-registered, verdict-ready
report of the fix in flight, so a null is reported as a finding about
spectrum-blind objectives, not hidden.
