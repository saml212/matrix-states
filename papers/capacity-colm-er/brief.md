...
| C17 | Held-out-axis correction (gauntlet r1 FIX-1): every h4 draws entities from the training pool; the held-out axis is hop depth only. The disjoint-entity diagnostic (`C17_heldout_entities`, anchor blend bypassed by construction per the registered scope limit) reads rec@0.9 = 1.0 at hop 1 in all 24 cells backing fig 1, and at hops 2-3 reads exactly 0.0 in 23 of 24 cells (one d=64 cell reads 0.0002), while trained-pool readouts stay at/near ceiling | `KEY_ANCHORING_DESIGN.md` §3.3 reporting requirement (~774) + §9.3 scope-limit passage (~1820) + M1 attack row (~1550: the anchoring fix "claims NO held-out-entity gains") | the SAME cell JSON sets as C1/C3 (concat-md5s 19ca8ed61c065710c627debb1ef3eb82, 53bc17fb892489a6b820f654c21607e2), field `checkpoints[-1].C17_heldout_entities` — ranges recomputed from raws this session | prose (limitations) |
