...
under-provisions larger states. Byte honesty is mandatory: between the two
LOCATED frontiers, capacity in raw bindings grows as d^1.97 (CI-propagated
two-point exponent range [1.86, 2.09]) — statistically indistinguishable
from proportional-to-state-BYTES (d^2) and excluding
proportional-to-dimension (d^1). The super-linear claim is per DIMENSION
(x0 = K*/d rises); per byte, capacity is approximately conserved over the
measured range. Both sides of this arithmetic are stated in the paper.
...
| C4 | d=128 instrument-saturation check: hop-21 recovery spans 0.9900–1.0 across the 12 cells (not uniformly 1.0); final-checkpoint effective rank tracks K (K=84 per-seed 83.7/83.6/83.4, mean 83.6) | `KEY_ANCHORING_DESIGN.md` §13.10 | the 12 cell JSONs of C3 (checkpoints[-1].M3_held_out['21'].recovered_frac@0.9; ['4'].effective_rank_whole_mean) — values recomputed from raws this session | prose (§ results) |
| C5 | Coherence framing: Welch bound forces max-abs-cos > 0 at n=107 > d=64 (construction value 0.2842, matched by the 0.284 dose arm; realized d=64 training band of final-checkpoint K-means [0.3731, 0.3845]); d=128 table exactly orthogonal at construction — the d=128 "0.000000" is DEFINITIONAL-DERIVED (n=107 <= d=128 permits an orthogonal frame; `anchor_table_init_mode="frame_potential"`), not a recorded field of the learned dstate cells (whose tables drift to 0.17-0.20 under training); corroborated by the dose cells, whose realized max_abs_cos matches the injected dose to 4 decimals (0.130 -> 0.13008, 0.284 -> 0.28402), i.e. the base table contributes ~0 (format audit r1 minor-2 tag) | `KEY_ANCHORING_DESIGN.md` §14.0b | d=64 band recomputed this session from the 12 cliff cell JSONs (C1 row, item6_table_conditioning.max_abs_cos, K-means 0.3845/0.3790/0.3789/0.3731); construction/dose value present in dose cell JSONs (C6 row) | fig2 (grey band) |
...
| C9 | Dose-wave instrument check: hop-21 recovery 0.9963–1.0 across all 19 dosed cells (not uniformly 1.0; the rank-4 arm alone spans 0.9987–1.0) | `KEY_ANCHORING_DESIGN.md` §14.12 | dose cell JSONs of C6/C7 (M3_held_out['21']) — range recomputed from raws this session | prose (§ dose) |
...
| C14 | Near-ceiling fine structure at d=96 is instrument-limited: a registered two-cell fresh-seed diagnostic routed to its pre-registered DEGENERATE_CELL verdict (neither cell admissible at the recalibrated gate; the failing admission leg differed from the frozen grid's), so the no-cliff claim is scoped as absence-of-monotonic-collapse, never as a settled value at K=90. AMENDED (gauntlet r1 FIX-2): §15.27's K=90 fresh reading h4=0.9725 (ceiling non-replication) MAY appear in the paper, but ONLY as a caveat that discounts the archived K=90 reading (Table 1 dagger + limitations sentence), never as a positive finding | `KEY_ANCHORING_SCALING_DRAFT.md` §15.27 (harvest record, 2026-07-08; the 0.9725 and inadmissible-at-n_iter-28 statements are verbatim there) | **No repo-archived raw** (cells box-side only; disclosed). All OTHER §15.27 descriptive numbers (shift magnitudes, salvage values) remain excluded | prose (limitations) + Table 1 dagger |
| C15 | Derived byte arithmetic (definitional, computed from C2/C10/C11 rows): a d x d fp32 state is 4d^2 bytes — 16 KiB (d=64), 25 KiB (d=80), 36 KiB (d=96), 64 KiB (d=128); located midpoint loads K* = x0*d: 34.9 bindings (d=64), 54.2 (d=80); d=96 holds >=90 bindings at ceiling within its measured window; two-point growth exponent p = ln(54.23/34.91)/ln(80/64) = 1.97, CI-propagated range [1.86, 2.09] (endpoints from the C2/C10 CI bounds) — consistent with proportional-to-bytes (p=2), excluding proportional-to-dimension (p=1); bindings per KiB at the located points: 2.18 (d=64), 2.17 (d=80), >=2.50 (d=96, bound) | derivation shown in-paper (arithmetic on C2/C10/C11 numbers; no new measurement) | C2/C10/C11 raw artifacts | table 1 |
| C16 | d=96 original registered grid (K=24/51/57/63/69, n=3): h4 K-means 1.0/1.0/0.9974/0.9805/0.9839 — flat near ceiling across K/d in [0.25, 0.71875]; fit degenerate (98.5% of 4,000 bootstrap resamples), CI null | `KEY_ANCHORING_SCALING_DRAFT.md` §15.19 (wave verdict; degenerate-disclosure rule applied) | `experiment-runs/2026-07-07_keyanchor_scaling/fit_cliff_curve_d96_results.json` md5:8f48aaddf77df460cf0f241b9232a559 | prose (§ frontier) |
| C17 | Held-out-axis correction (gauntlet r1 FIX-1): every h4 draws entities from the training pool; the held-out axis is hop depth only. The disjoint-entity diagnostic (`C17_heldout_entities`, anchor blend bypassed by construction per the registered scope limit) reads rec@0.9 = 1.0 at hop 1 in all 24 cells backing fig 1, exactly 0.0 at hop 2 in 23 of 24 cells (one d=64 cell reads 0.0002, raw 0.00017233, hop 2 only), and exactly 0.0 at hop 3 in all 24 cells (format audit r1 minor-3 split); the matched trained-pool comparison (`M2_in_distribution`) stays at ceiling (1.0) at hops 1-2 in all 24 cells but degrades at hop 3 under the highest tested d=64 loads (K=42 seed-mean 0.736, K=46 down to 0.56-0.60, worst single seed 0.5578) — gauntlet r1 FIX-9 (A8/A9 scoped-reattack findings) qualified the abstract/intro "held-out" phrasing to "held-out 4-hop" and corrected the limitations ceiling clause to name the hop-3 exception | `KEY_ANCHORING_DESIGN.md` §3.3 reporting requirement (~774) + §9.3 scope-limit passage (~1820) + M1 attack row (~1550: the anchoring fix "claims NO held-out-entity gains") | the SAME cell JSON sets as C1/C3 (concat-md5s 19ca8ed61c065710c627debb1ef3eb82, 53bc17fb892489a6b820f654c21607e2), fields `checkpoints[-1].C17_heldout_entities` and `checkpoints[-1].M2_in_distribution` — ranges recomputed from raws this session (round-1b scoped re-attack) | prose (limitations) |
| C18 | Anchor-table byte footprint (derived arithmetic, same convention as C15): 428*d_state bytes fp32 = 26.75 KiB (d=64, 67% above the 16 KiB state), 53.5 KiB (d=128, 84% of the 64 KiB state); O(n*d) vs the state's O(d^2); excluded from bindings/KiB (fixed by n, load-independent), disclosed as additive in any deployment budget | derivation shown in-paper (n=107 from C5; d_state the sweep variable); rebuttal r1 FIX-3 corrected the attack/defense 27.4 KiB figure to 26.75 KiB (/1024 convention) | none needed (definitional) | prose (testbed) |
| C20 | K=69/seed=1730 exclusion disclosure: a thirteenth examined cell (earlier wave, identical failing signature) deliberately NOT flipped per the declared 11-cell recalibration scope; that cell reads h4=0.9918 (raw: 0.9917912), so including it would RAISE the K=69 mean 0.9592 -> 0.9673, changing no conclusion. TIEBREAK RECORDED (format audit r1 CRITICAL, coordinator-verified vs raw): the design record's ~5141 hand-calc ("0.9592 -> 0.9423", implying a phantom fourth value of 0.8917) is WRONG against its own cited raw and direction-reversed; the paper prints the raw-derived 0.9673; the design doc is a read-only Source for this run and its correction is flagged to the coordinator, not edited here | `KEY_ANCHORING_SCALING_DRAFT.md` ~5133 (disclosed scope decision; its counterfactual VALUE superseded by the raw per the tiebreak above) | `experiment-runs/2026-07-08_c17_repro/fits/recalibrated_admissibility_table.json` md5:0877b840cc928dae04dff1929c2a9692 (the NOT-flipped K69/s1730 row, field h4_M3_hop4_recovered_frac_at_0.9 = 0.9917912483215332); 3-seed K=69 mean 0.9591542 from the C11 fit JSON md5:61eaffe1744a56086af2f4115f9a9cf4 | prose (§ frontier) |

Rule applied: every number in the draft carries an evidence comment
immediately after the claim; a number with no row does not enter the paper.
C14's box-side numbers are excluded by construction. **Mechanism adaptation
(recorded for the format auditor):** sections are authored directly as
LaTeX, where an HTML comment would typeset as body text, so the required
greppable token is the LaTeX comment `% evidence: Cn` at the end of the
claim's source line — same token grammar, comment syntax adapted to the
source language.
