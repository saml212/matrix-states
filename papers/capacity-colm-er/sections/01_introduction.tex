...
held-out 4-hop compositional recall does not decline gracefully with
load: it transitions sharply, and a logistic fit over the measured loads places the
...
the $d=64$ anchor table (the testbed's fixed per-entity key table;
Section~\ref{sec:testbed}) is forced non-orthogonal by the Welch bound while
