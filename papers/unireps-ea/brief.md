...
| U5 | L≥2 robustness: full-sample vs L≥2-only recovery near-identical per group (max \|Δ mean_cos\| 0.041 family-wide; per-group deltas ≤0.013) | §1.33 (schema deviation disclosed) | `harvest_analysis_output.txt` md5:854a4bd7c46e626badcc0fbf05d0e07a (per-group deltas) + the 19 unconstrained per-seed JSONs from the U1 row (`mean_cos`/`l_ge2_mean_cos` fields; the 0.041 per-seed maximum is max over seeds of \|mean_cos − l_ge2_mean_cos\|, attained at S4 seed 3) | §3 footnote + Appendix B text |
