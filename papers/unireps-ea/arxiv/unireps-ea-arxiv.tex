% arXiv NAMED build of the UniReps 2026 Extended Abstract (NeurIPS-kit
% neurips.sty). Derived from the pinned anonymized review bundle
% (bundle/unireps-ea-submission.tex) with exactly two changes, both
% de-anonymization: (1) the style file's [preprint] mode replaces the
% default submission mode (real author block renders, line numbers
% drop); (2) the real author block is filled in. The gauntleted body
...
% Blank the notice line (the 2025 kit's preprint mode prints "Preprint."
% by default; this named arXiv build carries no venue notice).
\makeatletter
...
\author{%
  Sam Larson\\
  Pebble AI (\url{https://pebbleml.com})\\
  \texttt{samlarson16@gmail.com}%
}
