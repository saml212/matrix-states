...
orderings. Two registered hypotheses die here at once: that a larger cache
would eventually hold the bindings (it does not; $M{=}32$ grants the cache
more bytes than the episode needs and the reading does not move), and that
forced locality might \emph{help} by concentrating attention on recent
tokens (it does not; every capped reading sits at or below uncapped).
