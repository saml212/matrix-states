...
\item \textbf{A constant-memory demonstration.} Recall accuracy is flat in
  context length out to eight times the binding span at a fixed
  32{,}768-byte state, while no KV budget up to $32\times$ those bytes
  rescues the transformer (Section~\ref{sec:horizon}).
