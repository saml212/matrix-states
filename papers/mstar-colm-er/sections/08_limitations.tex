...
non-competitiveness holds at the matched budget and across the
pre-registered strengthening search of Section~\ref{sec:recall}, up to
$3.1\times$ the matched parameters, $3\times$ the matched steps, and two
learning rates; % <!-- evidence: C12 -->
a transformer beyond that region, or one trained with a different
recipe, requires its own matched run before any extension of the claim. The metric is argmax-decoded and
