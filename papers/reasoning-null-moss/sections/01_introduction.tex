...
across scales and under targeted interventions. Those measurements carried
an unexamined promise: that write geometry matters for something a user
would recognize, such as composing multi-hop relations presented in
context. This paper reports the program that tested the promise; the test
produced a null. A reader of a null has two standing worries, that the
instrument never worked and that the analysis could declare a null after
seeing the data. Three pre-registered bounds address both:
...
(a 4.47-fold cohort variance mismatch against a pinned 4.0),
