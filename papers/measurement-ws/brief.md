...
| I2 | anchor-table NS residuals over 6,000× below the 0.01 tolerance, file-global across all six audited anchor tables at the operative n_iter=20 (max 1.5555e-06 → 6,429×; mean-of-means 1.182e-06 → 8,460×; format-audit M2's "max 1.416e-06 → 7,060×" and its own basis, sweep[0]/6 = table K78 s1840, were both correct only for that single table — final review §2.3 caught that §3's "across failing and passing cells" clause fixes the population at all six tables, two of which (K84 s1940 1.4679e-06; K90 s2040 1.5555e-06) exceed the quoted max; re-paired to the file-global max per final-review change 1) | §15.23 (MISDIAGNOSED-ARTIFACT) | `experiment-runs/2026-07-08_ns_admission_diag/results/diag_ns_admission_result.json` md5:d0d940d0afcdf30af8f45497168fb297 (`anchor_table_ns_sweep`, all 6 entries' `per_n_iter["20"]`, independently recomputed) | §3 text |
