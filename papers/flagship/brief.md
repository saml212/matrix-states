...
| R12 | **FIX-5 transformer task-1 LR-grid — §1.45 VERDICT OF RECORD: OUTCOME A, tuned transformer still below bar at every point.** Grid {1e-4,3e-4,1e-3,3e-3}×3 seeds×20,000 matched steps under the identical frozen protocol (3e-4 column reused verbatim from the R4 sweep, cited in §1.44); mean acc_A per lr: 0.0142/0.0283/0.0320/0.0283 (0.46×/0.91×/1.02×/0.91× chance); 0/12 individual cells and 0/4 grid means clear the 0.09375 bar; best = lr 1e-3 at 1.02× chance. DISSOCIATION FINDING: lr=1e-4 optimizes the LM objective best (final train loss 6.55–6.60 vs the default's ≈7.5) yet reads acc_A 6.3σ BELOW chance (n=4096, binomial σ=0.0027) — falsifies FIX-1's under-optimized-arm hypothesis; the two worse-optimizing rates (1e-3, 3e-3; loss ≈7.7–7.8) read closer to chance. Instrument health + ridge-harness sanity control PASS on all 9 fresh cells; re-metrics provenance-md5-pinned. §1.40 axis-1 WIN verdict UNCHANGED (this round is diagnostic/disclosure-grade; the ablation stays the registered carrier) | `HEAD_TO_HEAD_DEMO_DESIGN.md` §1.45 (verdict of record; §1.44 pre-run pre-registration); commit 440f7fd | `experiment-runs/2026-07-11_h2h_fix5_lrgrid/results/FIX5_LRGRID_VERDICT.json` md5:c71eacc8185bcec1ea27e86b20afd559 (+ 9 training JSONs + 9 re-metric JSONs + 1 smoke JSON, `md5_manifest.txt` 19/19 local==box verified; independently re-derived by this writer directly from the 9 `*_round4.json` `leg_a.acc_A` fields and the 9 training JSONs' `loss_final_mean5` field, not copied from the design-doc prose) | §7/§6/§1 abstract+intro prose (no figure) |
