...
0.344, 0.389, and 0.455 <!-- evidence: R6 -->: monotone in scale, with
...
4.489 <!-- evidence: R7 -->; all three paired seeds move in the
amplifying direction and an exploratory Welch test gives $p = 0.062$
<!-- evidence: R7 -->. We record gating as a direction-consistent trend
...
PARTIAL verdict at both scales <!-- evidence: R8 -->. At 98M the
...
and $+0.019$, a sign flip, on the other <!-- evidence: R8 -->: the
...
data mixes; 0 is the random-unit-vector anchor and 1 is full collapse,
...
the pre-registered $2\sigma$ bar over the corrected same-corpus noise
floor). Removing qk normalization is a within-noise null ($-0.103$,
$0.05\sigma$): the attractor is not a qk-normalization artifact. Gating
reads $+4.312$ ($1.92\sigma$, all three paired seeds positive), a
direction-consistent trend below the confirmation bar: neither a
confirmed amplification nor a null.

**Figure 7 caption.** Transfer of the frozen-key-bias mitigation to
scale: per-scale, per-corpus span-fraction deltas (deployed arm minus
eval-retrofitted comparator, $n{=}3$, 95 percent intervals; negative
would be stabilizing). At 98M both corpora read destabilizing with
intervals excluding zero; at 392M one corpus reads destabilizing and
one is null; no cell reverses. Validation-loss neutrality passes in all
eight gate cells (not shown), so the construction's cost neutrality
transfers while its geometric benefit does not. The 392M rung ran a
reduced token budget; cross-scale attenuation is therefore not a
registered claim.
