...
Covers:
  ...
  smoke 9: keyanchor_scaling_wide_kernel_gate_check -- POSITIVE against a
    fake-but-registered-shape PASSING artifact (the REAL artifact is
    box-only, sec 15.20.1's NEW probe not yet run) AND 5 NEGATIVE tests to
    completion (missing file, verdict without CLEARED, exit_code!=0,
...
LOCAL_K69_ARCHIVE_DIR = os.path.join(
    HERE, "..", "..", "experiment-runs", "2026-07-07_keyanchor_scaling",
    "results", "deltanet_rd_exactness", "wavekeyanchor-scaling")
...
def smoke_9_wide_kernel_gate_check():
    # The REAL artifact is box-only (sec 15.20.1's NEW probe not yet run this session) --
    # confirm the function reports a clean "not found" refusal, not a crash.
    real = rdx.keyanchor_scaling_wide_kernel_gate_check()
    real_reports_missing = real["ok"] is False and "not found" in real["reason"]
    print(f"    REAL artifact (box-only, not yet run): ok={real['ok']} "
          f"(expect False/'not found'): {real_reports_missing}")
    ...
    ok = real_reports_missing and pos_ok and neg_a_ok and neg_b_ok and neg_c_ok and neg_d_ok and neg_e_ok
    _report("smoke 9: keyanchor_scaling_wide_kernel_gate_check -- correctly reports the REAL "
            "artifact missing (box-only), PASSES a fake registered-shape artifact, and 5 NEGATIVE "
            "tests run to completion", ok)
