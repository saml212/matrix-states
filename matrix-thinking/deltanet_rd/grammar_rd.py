...
def sample_batch_rd(cfg: DeltaNetRDTaskConfig, batch_size: int, gen: torch.Generator,
    ...
    Returns:
      ...
      a_slot        (B, Q) int64         DELTANET_RD_EXACTNESS_DESIGN.md sec
                                          14.2 build item: the query's SOURCE
                                          slot (already computed below to derive
                                          tgt_slot/query_key_ids; now also
                                          RETURNED -- zero new compute, a pure
                                          dict-key addition). Always in [0,K)
                                          by construction (torch.randint(0,K,...)
                                          below) -- every query asks about hops
                                          from an entity that IS one of this
                                          episode's own K bound entities.
                                          F-geo-3's readout() gathers
                                          bind()'s own (orthogonalized)
                                          k_eff_items at this slot rather than
                                          recomputing q_eff via
                                          effective_key_window (sec 14.2: the
                                          orthogonalizing transform is a JOINT
                                          function of the K co-occurring keys,
                                          not recoverable from a query window
                                          in isolation).
    """
    ...
    return {
        ...
        "a_slot": a_slot,   # sec 14.2 build item -- see docstring above
    }
...
def _self_test() -> None:
    ...
    print("\n[grammar 3d] a_slot bookkeeping (DELTANET_RD_EXACTNESS_DESIGN.md sec 14.6 Wave -1 "
          "smoke item 5): a_slot is always in [0,K) (trivial from torch.randint's own range, "
          "checked anyway) and key_ids.gather(1, a_slot) == query_tokens[:, :, -3] for every "
          "sampled batch -- closes the query-path half of the T_bind-bookkeeping concern "
          "(the write-path half is model_rd.py's own R2-8/padding checks).")
    for hset in (cfg.H_train, cfg.H_test, cfg.H_extra):
        ba = sample_batch_rd(cfg, 24, gen, hop_set=hset, pools=pools)
        assert "a_slot" in ba, "sample_batch_rd no longer returns a_slot"
        assert ba["a_slot"].shape == (24, cfg.queries)
        assert (ba["a_slot"] >= 0).all() and (ba["a_slot"] < cfg.K).all(), \
            "a_slot out of [0,K) range"
        expected_query_key = torch.gather(ba["key_ids"], 1, ba["a_slot"])
        assert torch.equal(expected_query_key, ba["query_tokens"][:, :, -3]), \
            "key_ids.gather(1, a_slot) does not match query_tokens' own KEY slot (-3)"
    print("  a_slot in [0,K) at every draw; key_ids.gather(1,a_slot) == query_tokens[:,:,-3] "
          "exactly across H_train/H_test/H_extra")
