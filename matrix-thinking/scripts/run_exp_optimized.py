#!/usr/bin/env python3
"""
Optimized Matrix Thinker — Phase 1 Experiments
================================================
Optimization A: Shared gate/value between delta and gamma (1.25× speedup)
Optimization B: Low-rank multiplicative projections r=8 (2× speedup)
Optimization C: torch.compile (1.2× speedup)
Combined target: ~3× speedup with <5% quality cost.

Reports BPB (bits per byte) alongside PPL.
Same data, same training setup as Round 2 for direct comparison.

Usage:
  torchrun --standalone --nproc_per_node=8 run_exp_optimized.py
"""
...
CONFIG = {
    ...
    "n_probes": 32,
    "n_iterations": 8,
    "low_rank_r": 8,           # rank for multiplicative projections
    "max_len": 2048,
    "dropout": 0.1,
    ...
    "results_dir": "/toy_story_slam/results/exp_optimized",
    "num_workers": 8,
    "use_compile": True,
}
...
class RowThenColProjection(nn.Module):
    """Full-rank: silu(A @ M) @ B. Used for attention Q/K/V/O."""
    def __init__(self, d):
        super().__init__()
...
class LowRankRowThenCol(nn.Module):
    """Low-rank: silu((I + U_A@V_A) @ M) @ (I + U_B@V_B). For multiplicative layer.
    The identity is a buffer (not learned). The low-rank part is the perturbation."""
    ...
    def forward(self, M):
        A = self.I + self.U_A @ self.V_A  # identity + low-rank perturbation
...
class MatrixFrobeniusAttention(nn.Module):
    """Full-rank attention — Q/K/V/O are full RowThenCol projections."""
...
class OptimizedMultiplicativeLayer(nn.Module):
    """
    Optimization A: Shared gate/value between delta and gamma.
    Optimization B: Low-rank projections (r=8) for all multiplicative ops.

    Before: 6 full-rank RowThenCol (delta: gate,value,up × gamma: gate,value,up)
    After:  2 low-rank shared (gate,value) + 2 low-rank separate (delta_up,gamma_up)
    = 4 low-rank projections instead of 6 full-rank. ~3× cheaper.
    """
    def __init__(self, d, r=8, dropout=0.1):
        super().__init__()
        self.norm = MatrixRMSNorm(d)
        # Shared gate and value (the "what to modify" signal)
        self.shared_gate = LowRankRowThenCol(d, r)
        self.shared_value = LowRankRowThenCol(d, r)
        # Separate up projections (the "how to modify left vs right")
        self.delta_up = LowRankRowThenCol(d, r)
        self.gamma_up = LowRankRowThenCol(d, r)
        # Additive write
    ...
    def forward(self, M):
        ...
        # Shared gate and value
        gated = F.silu(self.shared_gate(M_n)) * self.shared_value(M_n)
        # Separate up projections for left (delta) and right (gamma)
        delta = self.delta_up(gated) * s
        gamma = self.gamma_up(gated) * s
...
class ThinkingBlock(nn.Module):
    def __init__(self, d, n_heads=8, r=8, dropout=0.1):
        super().__init__()
        self.attn = MatrixFrobeniusAttention(d, n_heads, dropout)
        self.think = OptimizedMultiplicativeLayer(d, r, dropout)
    def forward(self, M):
...
class OptimizedMatrixThinker(nn.Module):
    def __init__(self, mat_dim=32, n_layers=8, n_heads=8, n_probes=32, r=8,
        ...
        self.layers = nn.ModuleList([ThinkingBlock(d, n_heads, r, dropout) for _ in range(n_layers)])
...
def evaluate(model, val_ds, vocab_size, device, n_iterations, eval_batch_size=96, max_eval_batches=50):
    ...
    avg_bpb = avg_loss / math.log(2) / 3.7  # bits per byte
    avg_ranks = []
    if all_ranks:
        n_r = len(all_ranks[0])
...
def run():
    ...
    logger.log(f"Optimized Matrix Thinker | GPUs: {world_size}")
    logger.log(f"Optimizations: shared gate/value + low-rank r={cfg['low_rank_r']} + compile={cfg['use_compile']}")
    ...
    raw_model = OptimizedMatrixThinker(
        mat_dim=cfg["mat_dim"], n_layers=cfg["n_thinking_layers"],
        n_heads=cfg["n_heads"], n_probes=cfg["n_probes"], r=cfg["low_rank_r"],
    ...
    # Optimization C: torch.compile
    if cfg["use_compile"]:
        try:
            model = torch.compile(model, dynamic=False)
            logger.log("torch.compile: enabled")
        except Exception as e:
            logger.log(f"torch.compile failed: {e}, continuing without")
    ...
    while step < cfg["max_steps"]:
        ...
        if step % cfg["log_interval"] == 0:
            ...
            logger.log(f"Step {step:5d} | Loss {loss.item():.4f} | PPL {ppl:8.1f} | BPB {bpb:.3f} | "
            ...
            if is_main:
                ...
                for T in cfg["eval_iterations"]:
                    ...
                    if T == cfg["n_iterations"] and vl < best_val_loss:
                        ...
                        marker = " *BEST*"
                    logger.log(f"  T={T:2d}: PPL {vp:8.1f} | BPB {vbpb:.3f} | Rank [{rank_str}]{marker}")
    ...
    if is_main:
        logger.log(f"\n{'='*70}")
        logger.log(f"DONE: Optimized Matrix Thinker | {step} steps | {elapsed/60:.1f} min")
        ...
        for T in cfg["eval_iterations"]:
            vl, vp, vbpb, vr = evaluate(raw_model, val_ds, vocab_size, device, T, cfg["eval_batch_size"])
            rank_str = ", ".join(f"{r:.2f}" for r in vr) if vr else "-"
            logger.log(f"  T={T:2d}: PPL {vp:8.1f} | BPB {vbpb:.3f} | Rank [{rank_str}]")
        ...
        pct = delta_ppl / ppls[t1] * 100 if ppls[t1] > 0 else 0

        summary = "\n".join([
            "", "=" * 70,
            "  OPTIMIZED MATRIX THINKER — PHASE 1",
            "=" * 70,
            f"  Optimizations: shared gate/value + low-rank r={cfg['low_rank_r']} + compile",
            ...
            "",
            *[f"  T={T:2d}: PPL {ppls[T]:8.1f} | BPB {bpbs[T]:.3f}" for T in cfg["eval_iterations"]],
            "",
            f"  Thinking benefit: {delta_ppl:.1f} PPL ({pct:.1f}%)",
            f"  T=1 BPB: {bpbs[t1]:.3f} → T=8 BPB: {bpbs[tn]:.3f}",
            "",
            f"  vs Round 2 baseline: PPL 72.4 / BPB 1.67 at T=8",
            "=" * 70, ""
        ])
        logger.log(summary)
        with open(results_dir / "SUMMARY.txt", 'w') as f: f.write(summary)
        json.dump({"experiment": "exp_optimized", "params": n_params,
                    ...
                    "optimizations": ["shared_gate_value", f"low_rank_r{cfg['low_rank_r']}", "torch_compile"]},
