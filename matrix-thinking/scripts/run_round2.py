#!/usr/bin/env python3
"""
Round 2: Matrix Thinker with Matrix-Native Output Head
=======================================================
Key change from Round 1: MultiProbeHead replaces vector collapse.
The output now preserves full matrix structure — no flattening anywhere.

score_w = sum_k out_weight[w,k] * u_k^T M v_k

This probes both row AND column structure of the matrix. The current head
only probes rows (collapses columns via sum). 18x more column gradient
discrimination in benchmarks.

All 8 GPUs via DDP. Big reasoning data (2B+ tokens).

Usage:
  torchrun --standalone --nproc_per_node=8 run_round2.py
"""
...
CONFIG = {
    ...
    "n_thinking_layers": 8,
    "n_heads": 8,
    "n_probes": 32,            # MultiProbeHead: K=d for full expressiveness
    "n_iterations": 8,
    "max_len": 2048,
    "dropout": 0.1,
    "batch_size_per_gpu": 96,     # ~43GB/80GB VRAM, safe margin for eval
    "seq_len": 512,
    "max_steps": 3000,
    "lr": 3e-4,
    "warmup_steps": 300,
    ...
    "results_dir": "/toy_story_slam/results/round2_multiprobe",
    "num_workers": 8,
}
...
class MultiProbeHead(nn.Module):
    ...
    No vector collapse. Preserves both row and column structure."""
    ...
    def forward(self, M):
        ...
        return self.out(probes)                                # (B,L,vocab)
...
class MatrixThinker(nn.Module):
    def __init__(self, mat_dim=32, n_layers=8, n_heads=8, n_probes=32,
    ...
    def forward(self, token_ids, n_iterations=1, measure_ranks=False):
        ...
        logits = self.output_head(M_n)  # Matrix-native — NO vector collapse
...
# ═══════════════════════════════════════════════════════════════════════════════
# DATA + EVAL + TRAINING (same structure as round 1)
# ═══════════════════════════════════════════════════════════════════════════════
...
def evaluate(model, val_ds, vocab_size, device, n_iterations, eval_batch_size=96, max_eval_batches=50):
    ...
    avg_bpb = avg_loss / math.log(2) / 3.7  # bits per byte (GPT-2 ≈ 3.7 bytes/token)
    avg_ranks = []
    if all_ranks:
        n_r = len(all_ranks[0])
...
def run():
    ...
    logger.log(f"Round 2: Matrix Thinker + MultiProbeHead | GPUs: {world_size}")
    logger.log(f"Config: {json.dumps(cfg, indent=2)}")

    # Data — use big data if available, fall back to wiki
    ...
    logger.log(f"Sources: {meta.get('sources', '?')}")
    ...
    logger.log(f"Output head: MultiProbeHead K={cfg['n_probes']} (matrix-native, no vector collapse)")
    ...
    if is_main:
        logger.log(f"\n{'='*70}")
        logger.log(f"DONE: Round 2 MultiProbeHead | {step} steps | {elapsed/60:.1f} min")
        ...
        summary = "\n".join([
            "", "=" * 70,
            "  ROUND 2: MATRIX THINKER + MULTIPROBE OUTPUT",
            "=" * 70,
            f"  Params: {n_params:,} | {elapsed/60:.1f} min on {world_size}x H100",
            f"  Output: MultiProbeHead K={cfg['n_probes']} (NO vector collapse)",
        ...
        json.dump({"experiment": "round2_multiprobe", "params": n_params,
