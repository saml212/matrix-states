...
(Table~\ref{tab:pc-negctrl}; Fig.~\ref{fig:pc-negctrl} overlays the
per-seed and pooled curves on the matrix-CODI flatten reference). As a
sensitivity floor, replacing $h$ at the same six positions with i.i.d.\
Gaussian noise matched in mean and standard deviation produces seed
accuracies $\{79.6, 79.2, 78.2\}$pp --- statistically indistinguishable
from the unablated $\{80.0, 78.8, 78.0\}$pp. The intervention paradigm
...
\caption{Negative control: rank-$k$ ablation on vanilla GPT-2 SFT
(no matrix bottleneck, no $\Z$). Each gray dotted line is one seed of
the vanilla model; the heavy black line is the pooled mean across the
three seeds. The matrix-CODI flatten curve from \S\ref{sec:flat} is
overlaid in red as the reference. Both curves are flat to within a
fraction of a percentage point. The probe alone does not distinguish
between rank-blindness in matrix-CODI and position-irrelevance in a
model with no $\Z$.}
