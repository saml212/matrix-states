...
to $\Z$ does not depend on $\Z$:
\[
\frac{\partial\phi(\Z)}{\partial\Z_{ij}} \;=\; W_{\text{down}}[:,\,ij].
\]
By the chain rule, the loss gradient through the readout is
\[
...
\]
a vector contracted with a constant tensor. The contraction
preserves the row-space of $W_{\text{down}}$ but carries no
information about the singular structure of $\Z$.
