...
\begin{itemize}
  ...
  \item \textbf{This fix's own regime is now bounded, not open-ended ---
    ...
    $d_{\mathrm{state}}{=}80$ places it at $x_0{=}0.6756$ (95\% CI
    $[0.6620,0.6868]$), cleanly excluding the ratio-invariant band around
    $0.5455$; at $d_{\mathrm{state}}{=}96$ the transition does not
    localize inside the tested window at all (load tolerance $>0.72$).
    The exact functional form and the precise $d{=}96$ transition point
    remain open --- a wide-grid follow-up is designed and power-checked
    but not yet built or launched (\S\ref{sec:discussion}).
