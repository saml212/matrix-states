...
\begin{itemize}
  ...
  \item \textbf{This fix's own regime is now bounded, not open-ended.}
    Extending the identical design to $K{=}48$ ($K/d{=}0.75$) shows both
    named explanatory channels measurably active yet the fix still misses
    its own bar 0/3 seeds --- the capacity curve completes as a cliff
    ($\sim$1.00 $\to$ $\sim$0.65 $\to$ $\sim$0.02 at
