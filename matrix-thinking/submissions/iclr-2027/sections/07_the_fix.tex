...
\subsection{The substitute admission-stack caveat (state VERBATIM, not paraphrased)}
\begin{itemize}
  ...
  \item \textbf{Comparability is UNVERIFIED, stated plainly, every place a
    geo3 seed count appears:} the two arms' admission stacks differ, what
    each filters, and the realized pass rate under each must be named ---
    a geo3 ``$\geq$2 clean seeds'' is NOT the same evidentiary event as a
    learned-arm ``$\geq$2 clean seeds.''
\end{itemize}
